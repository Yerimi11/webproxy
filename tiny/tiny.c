...
int main(int argc, char **argv) { // argc : 옵션의 개수, argv : 옵션 문자열의 배열(도메인 들어감?)
  int listenfd, connfd;
  ...
  while (1) { /* 무한 서버 루프 실행 */
    ...
    // Getaddrinfo는 호스트 이름: 호스트 주소, 서비스 이름: 포트 번호의 스트링 표시를 소켓 주소 구조체로 변환 (모든 프로토콜에 대해)
...
void doit(int fd) { /* 한 개의 HTTP 트랜잭션을 처리 */
  ...
  Rio_readlineb(&rio, buf, MAXLINE); // 요청 라인 읽고 분석 // rip에 있는 string한 줄을 버퍼로 다 옮긴다
  printf("Request headers:\n");
  printf("%s", buf); // 우리가 요청한 자료를 표준 출력 해준다 (godzilla)
  sscanf(buf, "%s %s %s", method, uri, version);
  printf("Get image file uri : %s\n", uri); // 추가코드
  if (strcasecmp(method, "GET")) { // Tiny는 GET메소드만 지원. 만약 다른 메소드(like POST)를 요청하면. strcasecmp : 문자열비교.
  ...
  read_requesthdrs(&rio); // GET method라면 그건 읽고, 다른 요청 헤더들은 무시 (그냥 프린트한다)
  ...
  if (is_static) { /* Serve static content */ // 만일 요청이 정적 컨텐츠를 위한 것이라면
    ...
    serve_static(fd, filename, sbuf.st_size); // 만일 맞다면 정적 컨텐츠를 클라이언트에게 제공
  }
    ...
    serve_dynamic(fd, filename, cgiargs); // 만일 그렇다면 진행해서 동적 컨텐츠를 클라이언트에게 제공한다
  }  
}
